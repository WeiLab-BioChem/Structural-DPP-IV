...
class SeqLightningModule(LightningModule):
    ...
    def configure_loss(self):
        if self.args.loss == 'CE':
            ...
            if self.args.alpha:
                ...
                # alpha = torch.tensor(self.args.alpha)
            self.loss = FocalLoss.FocalLoss(self.args.num_class, alpha=alpha, gamma=self.args.gamma)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        datas, labels = batch
        logits, embedding = self.model(datas)
    ...
    def get_sklearn_metrics(self, logits, labels):
        ...
        # print(target)
        # print(pred_label)
        ConfusionMatrix = confusion_matrix(target, pred_label, labels=[0, 1])
        ...
        a1 = Tensor.cpu(torch.tensor(target))
        a2 = Tensor.cpu(pred)
        # print("a1", a1)
        # print("a2", a2)
        ...
        F3 = fbeta_score(target, pred_label, beta=3, labels=[0, 1])
        SE = recall_score(target, pred_label, labels=[0, 1])
        if TN + FP == 0:
            SP = 0
        else:
            SP = TN / (TN + FP)
